...
Program buildProgram(const GLchar* vsSrc, const GLchar* fsSrc) {
	ShaderTexture vs(GL_VERTEX_SHADER);
	vs.setSource(vsSrc);
	...
	ShaderTexture fs(GL_FRAGMENT_SHADER);
	fs.setSource(fsSrc);
...
Program loadProgram(const char* vsFile, const char* fsFile) {
    ShaderTexture vs = loadShaderTexture(GL_VERTEX_SHADER, vsFile);
	ShaderTexture fs = loadShaderTexture(GL_FRAGMENT_SHADER, fsFile);
